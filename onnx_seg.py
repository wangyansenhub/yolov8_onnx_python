...
class YOLOv8Seg:
    ...
    def __call__(self, im0, conf_threshold=0.9, iou_threshold=0.45, nm=32):
        """
    ...
    def draw_and_visualize(self, im, bboxes, segments, name, vis=False, save=True):
        """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default=r'./best_xray.onnx' ,help="Path to ONNX model")
    parser.add_argument("--source", type=str, default=r"./mei", help="Path to input image")
    ...
    # Read image by OpenCV
        ...
        print(f" {i}: ---Detected {len(boxes)} objects. {boxes}")
        # Draw bboxes and polygons
        if len(boxes) > 0:
            model.draw_and_visualize(img, boxes, segments, vis=False, save=True, name=i)
